...
int main(int narg, char * argv[])
{
	  ...
	  printf("i : %d\n",i);
		...
		std::cout << "Direction Bef " << -state.direction[0] << " " << -state.direction[1] << " " << -state.direction[2] << std::endl;

                /* Transport the muon backwards */
                pumas_transport(context, &state);
		
		std::cout << "Direction " << -state.direction[0] << " " << -state.direction[1] << " " << -state.direction[2] << std::endl;
